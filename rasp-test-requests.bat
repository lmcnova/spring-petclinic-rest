@echo off
REM =============================================================================
REM RASP Java Agent - API Test Requests
REM =============================================================================
REM Tests the RASP agent protection on Spring PetClinic REST API.
REM Run: rasp-test-requests.bat
REM
REM Prerequisites:
REM   - Spring PetClinic running on http://localhost:9966 with RASP agent attached
REM =============================================================================

set BASE_URL=http://localhost:9966/petclinic/api
...
REM -----------------------------------------------------------------------------
REM SECTION 1: Normal API Requests (should pass through)
REM -----------------------------------------------------------------------------
...
curl -s -w "\nHTTP Status: %%{http_code}\n" -X POST "%BASE_URL%/owners" -H "Content-Type: application/json" -d "{\"firstName\":\"Test\",\"lastName\":\"RaspUser\",\"address\":\"123 Test St\",\"city\":\"TestCity\",\"telephone\":\"1234567890\"}"
echo.
echo ---

echo ^>^> PUT /owners/1 - Update an owner
curl -s -w "\nHTTP Status: %%{http_code}\n" -X PUT "%BASE_URL%/owners/1" -H "Content-Type: application/json" -d "{\"firstName\":\"George\",\"lastName\":\"Franklin\",\"address\":\"110 W. Liberty St.\",\"city\":\"Madison\",\"telephone\":\"6085551023\"}"
echo.
echo ---
...
REM -----------------------------------------------------------------------------
REM SECTION 2: SQL Injection Attacks (RASP should detect at JDBC sink)
REM -----------------------------------------------------------------------------
...
curl -s -w "\nHTTP Status: %%{http_code}\n" -X POST "%BASE_URL%/owners" -H "Content-Type: application/json" -d "{\"firstName\":\"Robert'); DROP TABLE owners;--\",\"lastName\":\"Hacker\",\"address\":\"123 Evil St\",\"city\":\"Hackville\",\"telephone\":\"0000000000\"}"
echo.
echo ---
...
REM -----------------------------------------------------------------------------
REM SECTION 3: Path Traversal Attacks (RASP should detect at File sink)
REM -----------------------------------------------------------------------------
...
REM -----------------------------------------------------------------------------
REM SECTION 4: Command Injection Attacks (RASP should detect at Process sink)
REM -----------------------------------------------------------------------------
...
curl -s -w "\nHTTP Status: %%{http_code}\n" -H "X-Custom-Header: $(whoami)" "%BASE_URL%/owners"
echo.
echo ---
...
REM -----------------------------------------------------------------------------
REM SECTION 5: XSS Attacks (RASP should detect reflected payloads)
REM -----------------------------------------------------------------------------
...
curl -s -w "\nHTTP Status: %%{http_code}\n" -X POST "%BASE_URL%/owners" -H "Content-Type: application/json" -d "{\"firstName\":\"<img src=x onerror=alert(1)>\",\"lastName\":\"XSSTest\",\"address\":\"123 Test St\",\"city\":\"TestCity\",\"telephone\":\"1234567890\"}"
echo.
echo ---
...
REM -----------------------------------------------------------------------------
REM SECTION 6: IDOR Attacks (accessing other users' resources)
REM -----------------------------------------------------------------------------
...
REM -----------------------------------------------------------------------------
REM SECTION 7: Header Injection / HTTP Smuggling
REM -----------------------------------------------------------------------------
...
curl -s -w "\nHTTP Status: %%{http_code}\n" -H "Host: evil.com" "%BASE_URL%/owners"
echo.
echo ---

echo ^>^> CRLF injection via header
curl -s -w "\nHTTP Status: %%{http_code}\n" -H "X-Injected: value%%0d%%0aInjected-Header: evil" "%BASE_URL%/owners"
echo.
echo.

REM -----------------------------------------------------------------------------
REM SECTION 8: Mass Assignment / Unexpected Fields
REM -----------------------------------------------------------------------------
...
curl -s -w "\nHTTP Status: %%{http_code}\n" -X POST "%BASE_URL%/owners" -H "Content-Type: application/json" -d "{\"id\":999,\"firstName\":\"Admin\",\"lastName\":\"Hacker\",\"address\":\"123 Hack St\",\"city\":\"Hackville\",\"telephone\":\"0000000000\",\"role\":\"ADMIN\",\"isAdmin\":true}"
echo.
echo.
